<?php
    $bereich = 'Startseite';
    $pageTitle = 'Produkte von VieCode';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $sql = "INSERT INTO `drittanbieter_viecode-rechnung` (pluginname, url, preis) VALUES (:pluginname, :url, :preis)";
        $statement = $connection->prepare($sql);
        $statement->bindParam(':pluginname', $pluginname, PDO::PARAM_STR);
...
$alles_ausgeben = "SELECT * FROM `drittanbieter_viecode-rechnung`";
$statement = $connection->query($alles_ausgeben);
$ausgabe = $statement->fetchAll(PDO::FETCH_ASSOC);

// Extras abrufen
$extras_ausgeben = "SELECT * FROM `drittanbieter_viecode-rechnung` WHERE (`pluginname` IN ('Installations-/Upgradeservice', 'BrandingFree Lizenz'))";
$statement = $connection->query($extras_ausgeben);
$extra_ausgabe = $statement->fetchAll(PDO::FETCH_ASSOC);

// Statistik ausgeben
$gezielt_ausgeben = "SELECT COUNT(*) AS produkt, SUM(preis) AS gesamtwert FROM `drittanbieter_viecode-rechnung`";
...
$wertvollste_produkte = "SELECT pluginname, preis FROM `drittanbieter_viecode-rechnung` ORDER BY preis DESC LIMIT 5";

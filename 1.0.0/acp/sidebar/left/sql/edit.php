...
$pageTitle = "Menüpunkt ändern (SQL-Aufgaben)";
require_once ($_SERVER['DOCUMENT_ROOT'] . "/layout/header/header.inc.php");

try {
    $id = $_GET['id'] ?? null;
    if ($id) {
        $sql = "SELECT * FROM sidebar_left_sql_aufgaben WHERE ID = :id";
...
<form action="edit.php?id=<?php echo $id; ?>" method="post">
    ...
    <input type="text" name="url" id="url" value="<?php echo htmlspecialchars($row['url']); ?>.php" required><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        if (!empty($_POST['url']) && !empty($_POST['ziel'])) {
            ...
            $prepare = $connection->prepare('UPDATE sidebar_left_sql_aufgaben SET url = :url, ziel = :ziel WHERE ID = :id');
            ...
            header("Location: /acp/sidebar/left/sqlindex.php");
            exit();
        } else {

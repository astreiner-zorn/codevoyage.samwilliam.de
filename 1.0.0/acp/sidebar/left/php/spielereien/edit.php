...
$pageTitle = "Menüpunkt ändern (Spielereien)";
require_once ($_SERVER['DOCUMENT_ROOT'] . "/layout/header/header.inc.php");

try {
    $id = $_GET['id'] ?? null;
    if ($id) {
        $sql = "SELECT * FROM php_sidebar_left_spielereien WHERE ID = :id";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        if (!empty($_POST['url']) && !empty($_POST['ziel'])) {
            ...
            $prepare = $connection->prepare('UPDATE php_sidebar_left_spielereien SET url = :url, ziel = :ziel WHERE ID = :id');

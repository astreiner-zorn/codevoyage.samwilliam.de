...
$pageTitle = "Menüpunkt ändern (Wissensportal)";
require_once ($_SERVER['DOCUMENT_ROOT'] . "/layout/header/header.inc.php");

try {
    $id = $_GET['id'] ?? null;
    if ($id) {
        $sql = "SELECT * FROM acp_sidebar_left_wissensportal WHERE ID = :id";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        if (!empty($_POST['url']) && !empty($_POST['ziel'])) {
            ...
            $prepare = $connection->prepare('UPDATE acp_sidebar_left_wissensportal SET url = :url, ziel = :ziel WHERE ID = :id');
            ...
            header("Location: /acp/sidebar/left/acp/index.php");
            exit();
        } else {

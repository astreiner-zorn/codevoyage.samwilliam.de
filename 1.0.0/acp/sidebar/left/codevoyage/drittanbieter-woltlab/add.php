...
$sql = "
CREATE TABLE IF NOT EXISTS `sidebar_left_drittanbieter_woltlab`
(
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        if (!empty($_POST['url']) && !empty($_POST['ziel'])) {
            ...
            $prepare = $connection->prepare('INSERT INTO `sidebar_left_drittanbieter_woltlab` (`url`, `ziel`) VALUES (:url, :ziel)');
            $prepare->bindParam(':url', $url, PDO::PARAM_STR);
            $prepare->bindParam(':ziel', $ziel, PDO::PARAM_STR);
            $prepare->execute();

            header("Location: /acp/sidebar/left/codevoyage/index.php");
            exit();
        } else {

...
try {
    $id = $_GET['id'] ?? null;
    if ($id) {
        $sql = "SELECT * FROM php_sidebar_left_eigene_werke WHERE ID = :id";
...
<form action="edit.php?id=<?php echo $id; ?>" method="post">
    ...
    <input name="url" id="url" value="<?php echo htmlspecialchars($row['url']); ?>" required><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        if (!empty($_POST['url']) && !empty($_POST['ziel'])) {
            ...
            $prepare = $connection->prepare('UPDATE php_sidebar_left_eigene_werke SET url = :url, ziel = :ziel WHERE ID = :id');
